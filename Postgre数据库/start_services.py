...
def start_service(script_name, port, service_name):
    ...
    try:
        ...
        if result == 0:
            print(f"⚠️ 端口 {port} 已被占用，跳过启动 {service_name}")
            return None
...
def main():
    ...
    try:
        ...
        for service in services:
            script_path = current_dir / service['script']
        ...
        print("   - API测试: 打开 test_api.html 文件")
        print("\n💡 使用说明:")
        print("   1. 在浏览器中打开 index.html")
        print("   2. 点击'刷新表列表'按钮")
        print("   3. 选择数据库表查看数据")
        print("   4. 点击'开始数据分析'按钮")
        print("   5. 查看分析结果和图表")
        print("\n⏹️ 按 Ctrl+C 停止所有服务")
        print("="*60)
